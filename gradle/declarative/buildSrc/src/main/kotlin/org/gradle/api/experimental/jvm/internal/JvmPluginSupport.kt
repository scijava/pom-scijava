...
object JvmPluginSupport {
    ...
    fun linkJavaVersion(project: Project, dslModel: HasJavaTargets) {
        val java = project.extensions.getByType<JavaPluginExtension>()
        java.toolchain.languageVersion = project.provider<JavaLanguageVersion> { JavaLanguageVersion.of(dslModel.targets.withType<JavaTarget>().map { it.javaVersion }.min()) }
    }
